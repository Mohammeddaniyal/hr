...
ResultSet resultSet;
/*
we can also check designation code existence by DesignationDAO
but the important thing is that another or reduntant connection will establish
...
throw new DAOException("Basic salary cannot be negative");
}
...
throw new DAOException("Pan number length cannot be zero");
}
...
throw new DAOException("Aadhar card number cannot be zero");
}
...
preparedStatement.close();
boolean aadharCardNumberExists=false;
preparedStatement=connection.prepareStatement("select gender from employee where aadhar_card_number=? adn employee_id<>?");
preparedStatement.setString(1,aadharCardNumber);
preparedStatement.setInt(2,actualEmployeeId);
resultSet=preparedStatement.executeQuery();
aadharCardNumberExists=resultSet.next();
...
resultSet.close();
preparedStatement.close();
preparedStatement=connection.prepareStatement("select * from employee from employee_id=?");
preparedStatement.setInt(1,designationCode);
resultSet=preparedStatement.executeQuery();
...
employeeDTO.setIsIndian(resultSet.getBoolean("is_indian"));
employeeDTO.setPANNumber(resultSet.getString("panNumber").trim());
employeeDTO.setAadharCardNumber(resultSet.getString("aadharCardNumber").trim());
employees.add(employeeDTO);
}
resultSet.close();
preparedStatement.close();
...
System.out.println("Designation is alloted : "+designationAlloted+",Code  : "+designationCode);
